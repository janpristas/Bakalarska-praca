...
#include "../../development/include/GXDLMSMyObject.h"
...
int GetIpAddress(std::string& address, std::string& subnet) {

    int ret;
    char str[INET_ADDRSTRLEN];

    int sock;
    struct ifreq interfc;
    struct sockaddr_in *addr;
    struct sockaddr_in *mask;


    sock= socket(AF_INET, SOCK_STREAM, 0);
    memcpy(interfc.ifr_name, "wlp7s0", IF_NAMESIZE);
    ioctl(sock, SIOCGIFADDR, &interfc, sizeof(interfc));

    addr = (struct sockaddr_in *)&interfc.ifr_addr;
    mask = (struct sockaddr_in *)&interfc.ifr_netmask;

    address = inet_ntop(AF_INET, &(addr->sin_addr.s_addr), str, INET_ADDRSTRLEN);
    subnet = inet_ntop(AF_INET, &mask->sin_addr, str, INET_ADDRSTRLEN);
    close(sock);
...
void AddElectricityID3(CGXDLMSObjectCollection& items, unsigned long sn)
{
    CGXDLMSVariant id3(sn);
    CGXDLMSData* d = new CGXDLMSData("1.1.0.0.2.255");
    d->SetValue(id3);
    d->GetAttributes().push_back(CGXDLMSAttribute(2, DLMS_DATA_TYPE_UINT32));
    items.push_back(d);
}
...
//     pActivity->SetCalendarNameActive("ActiveActivityCalendar");
...
//     pActivity->SetCalendarNamePassive("PassiveActivityCalendar");
...
void AddOpticalPortSetup(CGXDLMSObjectCollection& items)
{
    ...
    pOptical->SetProposedBaudrate(DLMS_BAUD_RATE_2400);
    pOptical->SetDefaultBaudrate(DLMS_BAUD_RATE_600);
    pOptical->SetResponseTime(DLMS_LOCAL_PORT_RESPONSE_TIME_20_MS);
    pOptical->SetDeviceAddress("Gurux");
    pOptical->SetPassword1("PassWd1");
    pOptical->SetPassword2("PassWd2");
    pOptical->SetPassword5("PassWd3");
...
void AddDemandRegister(CGXDLMSObjectCollection& items)
{
    ...
    pDr->SetCurrentAvarageValue(5);
    pDr->SetLastAvarageValue(25);
    pDr->SetStatus(1);
...
void AddActionSchedule(CGXDLMSObjectCollection& items)
{
    ...
    pActionS->SetExecutedScriptLogicalName("4.2.42.5.3.63");
...
void AddModemConfiguration(CGXDLMSObjectCollection& items)
{
    ...
    pMc->SetCommunicationSpeed(DLMS_BAUD_RATE_19200);
...
void AddMacAddressSetup(CGXDLMSObjectCollection& items)
{
    ...
    pMac->SetMacAddress(buff);
    items.push_back(pMac);
}

void AddMyObject(CGXDLMSObjectCollection& items)
{
    CGXDLMSMyObject* pObject = new CGXDLMSMyObject();
    pObject->SetValue("123");
...
*/
CGXDLMSIp4Setup* AddIp4Setup(CGXDLMSObjectCollection& items, std::string& address, std::string& subnet)
{
    CGXDLMSIp4Setup* pIp4 = new CGXDLMSIp4Setup("0.1.1.0.0.255");
    pIp4->SetIPAddress(address);
    // pIp4->SetSubnetMask((unsigned long) "255.255.0.0");
    // pIp4->SetGatewayIPAddress();
    pIp4->SetUseDHCP(0);
...
int CGXDLMSBase::Init(int port, GX_TRACE_LEVEL trace)
{
    ...
    profileGeneric->SetSortMethod(DLMS_SORT_METHOD_LIFO);
    ...
    ////////////////////////////////////////////////////////////////////////
    // Add my object
    AddMyObject(GetItems());

    ///////////////////////////////////////////////////////////////////////
    //Add IP4 Setup object.
    CGXDLMSIp4Setup* pIp4 = AddIp4Setup(GetItems(), address, subnet);
